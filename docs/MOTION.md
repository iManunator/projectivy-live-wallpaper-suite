...
Intensity presets **Subtle / Cinematic / Bold** (0.16 / 0.55 / 0.96) change **background** zoom and pan enough to see on a TV (same amplitude as the CSS preview). Duration defaults are longer (quality `light` / `standard` / `cinematic` ≈ 8 / 12 / 16s, clamp 2–24s). Bake uses a **CSS-like ping-pong** (ease in at the start, peak mid-loop, ease out to a seamless join) — it does **not** zoom out past the cover crop. Plate motion is **subpixel bicubic** (Pillow `EXTENT` on a 2× working plate) — not ffmpeg `zoompan`, which is nearest-neighbour and stair-steps slow pans. Default **30 fps** CFR, H.264 Main @ L4.0 `yuv420p` `+faststart`, **no B-frames**, **no scenecut**. Parallax may add a padded **light-leak** wash on the **background** (under locked chrome, never scraping the frame edge — not title chrome, not vignette). Unknown styles (including the typo “parrallelx”) normalize to **parallax**.
...
The Ken Burns path matches the editor CSS loop: zoom **in** only (never below the cover crop), ping-pong ease (`sin(πt)²`, not a bipolar `sin(2πt)` zoom-out), zoom/pan amplitude from the same `--motion-zoom-*` / `--motion-x` numbers as `web/src/lib/motion.ts`, default **30 fps**. Frames are resampled with **subpixel bicubic** windows (CSS-like), then encoded **CFR** with **B-frames off** and **scenecut off** so a TV loop does not hitch at a mid-pan keyframe or at the join. Quality presets pick an x264 speed (`fast` / `medium` / `slow`) and a target bitrate (2.8 / 4 / 5.5 Mbps) with 2× VBV maxrate headroom, always **H.264 Main 4.0 yuv420p +faststart** for Projectivy / Android TV. Duration defaults stay 8 / 12 / 16s (quality) so the loop is not shorter than the CSS preview.
